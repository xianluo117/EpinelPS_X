...
namespace EpinelPS.LobbyServer.Gacha
{
    ...
    public class ExecGacha : LobbyMsgHandler
    {
		...
		{
			GachaRate rate = GachaProbabilityConfig.Current.Normal;
			(double rRate, double srRate, double ssrRate) = GachaProbabilityConfig.GetNormalizedRates(rate);
			...
			double increasedChance = increasedChanceCharacterID != 1 ? (isPilgrimOrOverspec ? rate.IncreasedChancePilgrim : rate.IncreasedChance) : 0.0;
			increasedChance = GachaProbabilityConfig.ClampPercent(increasedChance);

			double roll = random.NextDouble() * 100; // Roll from 0 to 100

			if (roll < rRate && availableRCharacters.Count != 0)
			{
				// R category
				return availableRCharacters[random.Next(availableRCharacters.Count)];
			}
			else if (roll < rRate + srRate && availableSRCharacters.Count != 0)
			{
				// SR category
			...
			else
			{
				...
				if (ssrRoll < GachaProbabilityConfig.ClampPercent(rate.PilgrimRate) && availablePilgrimCharacters.Count != 0)
				{
					// PILGRIM SSR
